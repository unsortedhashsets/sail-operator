...
** <<multi-primary---multi-network-ambient-mode,Multi-Primary - Multi-Network (Ambient Mode)>>
...
== Multi-Primary - Multi-Network (Ambient Mode)

These instructions install a https://istio.io/latest/docs/ambient/install/multicluster/multi-primary_multi-network/[ambient/multi-primary/multi-network] Istio deployment using the Sail Operator and Sail CRDs. **Before you begin**, ensure you complete the <<common-setup,common setup>>.

**Note:** Ambient mode requires Istio CNI to be installed with the `ambient` profile. See the link:../common/istio-ambient-mode.adoc#introduction-to-istio-ambient-mode[Introduction to Istio Ambient mode] for detailed instructions on creating the `IstioCNI` resource.

. Create `ztunnel` namespace on `cluster1`.
+
----
kubectl get ns ztunnel --context "${CTX_CLUSTER1}" || kubectl create namespace ztunnel --context "${CTX_CLUSTER1}"
----
...
. Create `Ztunnel` resource on `cluster1`.
+
----
kubectl apply --context "${CTX_CLUSTER1}" -f - <<EOF
...
. Create `ztunnel` namespace on `cluster2`.
+
----
kubectl get ns ztunnel --context "${CTX_CLUSTER2}" || kubectl create namespace ztunnel --context "${CTX_CLUSTER2}"
----
...
. Create `Ztunnel` resource on `cluster2`.
+
----
kubectl apply --context "${CTX_CLUSTER2}" -f - <<EOF
apiVersion: sailoperator.io/v1alpha1
...
. Label `helloworld` service in `cluster1` as global so that it can be accessed from other clusters in the mesh
+
----
kubectl label --context="${CTX_CLUSTER1}" svc helloworld -n sample istio.io/global="true"
----
...
. Label `helloworld` service in `cluster2` as global so that it can be accessed from other clusters in the mesh
+
----
kubectl label --context="${CTX_CLUSTER2}" svc helloworld -n sample istio.io/global="true"
----
...
kubectl delete ns istio-system --context="${CTX_CLUSTER2}" 
kubectl delete ns sample --context="${CTX_CLUSTER2}"
kubectl delete ns ztunnel --context="${CTX_CLUSTER1}"
kubectl delete ns ztunnel --context="${CTX_CLUSTER2}"
----
